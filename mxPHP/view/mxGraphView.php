<?php
/**
 * $Id: mxGraphView.php,v 1.101 2010/02/25 12:08:18 gaudenz Exp $
...
class mxGraphView extends mxEventSource
{
	...
	function validatePoints($parentState, $cell)
	{
	 	...
	 	if ($state != null)
	 	{
			$geo = $this->graph->getCellGeometry($cell);
	 		...
	 		if ($isEdge)
	 		{
	 			$source = $this->getVisibleTerminal($cell, true);
	 			
	 			if ($source != null &&
	 				!$model->isAncestor($source, $cell))
	 			{
	 				$p = $model->getParent($source);
	 				$pstate = $this->getState($p);

	 				$this->validatePoints($pstate, $source);
	 			}
	 			
	 			$target = $this->getVisibleTerminal($cell, false);
	 			
	 			if ($target != null &&
	 				!$model->isAncestor($target, $cell))
	 			{
	 				$p = $model->getParent($target);
	 				$pstate = $this->getState($p);
		 	...
		 	if ($isEdge || $model->isVertex($cell))
		 	{
		 		...
		 		$bb = $this->updateBoundingBox($state);
	 		...
	 		!$this->graph->isCellCollapsed($cell))
	 	{
	 		...
	 		for ($i = 0; $i < $childCount; $i++)
	 		{
	 			...
	 			$bounds = $this->validatePoints($state, $child); // recurse
	 			
	 			$minX = ($minX != null) ? min($minX, $bounds->x) : $bounds->x;
	 			$minY = ($minY != null) ? min($minY, $bounds->y) : $bounds->y;
	 ...
	 * Parameters:
	 * 
	 * parentState - <mxCellState> that represents the parent state.
	 * childState - <mxCellState> that represents the child state.
	 */
    function childMoved($parentState, $childState)
    {
        $cell = $childState->cell;
	 ...
	 * 
	 * Updates the absolute terminal points in the state.
	 */
	function setTerminalPoints(&$state)
	{
		$trl = $this->translate;
		$edge = $state->cell;
		$geo = $this->graph->getCellGeometry($edge);
		$pt = $geo->getTerminalPoint(true);
		
		if ($pt != null)
		{
			$pt = new mxPoint($this->scale *
				($trl->x + $pt->x + $state->origin->x),
				$this->scale *
				($trl->y + $pt->y + $state->origin->y));
		...
		if ($pt != null)
		{
			...
			if ($n > 1)
			{
				$state->absolutePoints[$n-1] = $pt;
			}
			else
			{
				array_push($state->absolutePoints, $pt);
			}
		}
	}
	 ...
	 */
	function updatePoints(&$state, $points, $source, $target)
	{
		if ($state != null)
		{
			$pts = array();
			array_push($pts, $state->absolutePoints[0]);
			$edgeStyle = $this->getEdgeStyle($state, $points, $source, $target);

			if ($edgeStyle != null)
			{
				$src =& $this->getState(
					$this->getTerminalPort($state, $source, true));
				$trg =& $this->getState(
					$this->getTerminalPort($state, $target, false));
			...
			else if ($points != null)
			{
				for ($i = 0; $i < sizeof($points); $i++)
				{
					...
					{
						$pt = $points[$i]->copy();
						array_push($pts, $this->transformControlPoint($state, $pt));
					}
				}
			}
			
			$n = sizeof($state->absolutePoints);
			array_push($pts, $state->absolutePoints[$n-1]);
	 ...
	 *
	 */
	function getEdgeStyle($edgeState, $points, $source, $target)
	{
		$edgeStyle = null;
		
		if ($source != null && $source === $target)
		{
			$edgeStyle = mxUtils::getValue($edgeState->style, mxConstants::$STYLE_LOOP);
		...
		else if (!mxUtils::getValue($edgeState->style, mxConstants::$STYLE_NOEDGESTYLE, false))
		{
			$edgeStyle = mxUtils::getValue($edgeState->style, mxConstants::$STYLE_EDGE);
		}
	 ...
	 * target - <mxCell> that represents the visual target.
	 */
	function updateTerminalPoints(&$state, $source, $target)
	{
		if ($target != null)
		{
	 ...
	 * 
	 * Updates the terminal point on the specified state using start as the
	 * source cell and end as the target cell of the edge.
	 */
	function updateTerminalPoint(&$state, &$start, &$end, $isSource)
	{
		$start = $this->getTerminalPort($state, $start, $isSource);
	 	$index = ($isSource) ? 0 : sizeof($state->absolutePoints)-1;
	 	$state->absolutePoints[$index] = $this->getPerimeterPoint(
	 		$state, $start, $end, $isSource);
	}
	...
	function getTerminalPort(&$state, &$terminal, $source)
	{
		...
		if ($id != null)
		{
			$tmp = $this->graph->model->getCell($id);

			// Only uses ports where a cell state exists
			if ($this->getState($tmp) != null)
			{
				$terminal = $tmp;
			}
		}
	 ...
	 * source cell and end as the target cell of the edge.
	 */
	function getPerimeterPoint(&$state, &$start, &$end, $isSource)
	{
		$point = null;
		$terminalState =& $this->getState($start);
		
		if ($terminalState != null)
		{
			$perimeter = $this->getPerimeterFunction($terminalState);
			$next = $this->getNextPoint($state, $end, $isSource);
			
			if ($perimeter != null && $next != null)
			{
				$bounds = $this->getPerimeterBounds($terminalState, $state, $isSource);
				
				if ($bounds->width > 0 || $bounds->height > 0)
				{
					$point = $perimeter->apply($bounds,	$state, $terminalState,	$isSource, $next);
				}
			}
	...
	function getPerimeterBounds($terminal, $edge, $isSource)
	{
		...
		if ($edge != null)
		{
			...
			$border += mxUtils::getNumber($edge->style,
	 ...
	 * 
	 * Returns the nearest point in the absolute points of the specified
	 * state of the center of the opposite cell's state.
	 */
	function getNextPoint(&$state, &$opposite, $isSource)
	{
		$point = null;
		...
		{
			$count = sizeof($pts);
			$index = ($isSource) ? min(1, $count-1) : max(0, $count-2);
			$point = $pts[$index];
		}
		if ($point == null && $opposite != null)
		{
			$oppositeState =& $this->getState($opposite);
			if ($oppositeState != null)
			{
				$point = new mxPoint(
					$oppositeState->getCenterX(),
	 ...
	 * 
	 * Returns the nearest ancestor terminal that is visible. The edge
	 * appears to be connected to this terminal on the display.
	 */
	function getVisibleTerminal(&$edge, $isSource)
	{
	 	$model = $this->graph->model;
	 	$result = $model->getTerminal($edge, $isSource);
	 	$best = $result;
