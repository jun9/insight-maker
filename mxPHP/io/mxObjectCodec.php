<?php
/**
 * $Id: mxObjectCodec.php,v 1.14 2010/01/02 09:45:15 gaudenz Exp $
...
class mxObjectCodec
{
	...
	function encodeValue($enc, &$obj, $name, &$value, $node)
	{
		if ($value != null)
		{
			if ($this->isReference($obj, $name, $value, true))
			{
				...
				if (!isset($tmp))
				{
			    	mxLog::warn("mxObjectCodec.encode: No ID for value of ".
			    		mxCodecRegistry::getName($obj).".$name of type ".get_class($value));
			    	return; // exit
			    }
			...
			$defaults = get_object_vars($this->template);
	...
	function writeComplexAttribute($enc, &$obj, $attr, &$value, $node)
	{
    	...
    	else
    	{
    		mxLog::warn("mxObjectCodec.encode: No node for value of ".
    			mxCodecRegistry::getName($obj).".$attr");
    	}
	}
	...
	function decode($dec, $node, &$into = null)
	{
		...
		$obj = (array_key_exists($id, $dec->objects)) ? $dec->objects[$id] : null;
			...
			if ($this->isReference($obj, $fieldname, $value, false))
			{
				...
				if ($tmp == null)
				{
			    	mxLog::warn("mxObjectCodec.decode: No object for ".
			    		mxCodecRegistry::getName($obj).".$name=$value");
			    	return; // exit
			    }
